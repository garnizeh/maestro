...
func execRootForPull(h *Handler, args ...string) (string, error) {
...
func TestPullCmd_HelpFlag(t *testing.T) {
	h := NewHandler()
	out, err := execRootForPull(h, "pull", "--help")
	if err != nil {
...
func TestPullCmd_Success(t *testing.T) {
	h := NewHandler()
	h.PullDrawFn = func(_ context.Context, _, _ string, _ maturin.DrawOptions) error { return nil }

	out, err := execRootForPull(h, "pull", "nginx:latest")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestPullCmd_Success_Quiet(t *testing.T) {
	...
	out, err := execRootForPull(h, "--quiet", "pull", "nginx:latest")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if strings.Contains(out, "Pull complete") {
...
func TestPullCmd_DrawError(t *testing.T) {
	h := NewHandler()
	h.PullDrawFn = func(_ context.Context, _, _ string, _ maturin.DrawOptions) error {
		return errors.New("registry down")
	}

	_, err := execRootForPull(h, "pull", "nginx:latest")
	if err == nil {
...
func TestPullCmd_WithExplicitRoot(t *testing.T) {
	...
	h.PullDrawFn = func(_ context.Context, root, _ string, _ maturin.DrawOptions) error {
		capturedRoot = root
		return nil
	}

	if _, err := execRootForPull(h, "--root", "/tmp/maestro-test", "pull", "nginx:latest"); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestPullCmd_WithPlatformFlag(t *testing.T) {
	...
	h.PullDrawFn = func(_ context.Context, _, _ string, opts maturin.DrawOptions) error {
		capturedPlatform = opts.Platform
		return nil
	}

	if _, err := execRootForPull(h, "pull", "--platform", "linux/arm64", "nginx:latest"); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestPullCmd_DefaultRoot_UsesHomeDir(t *testing.T) {
	...
	if _, err := execRootForPull(h, "pull", "nginx:latest"); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
